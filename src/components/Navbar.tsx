      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-3.5">
        ...
        <div className="flex items-center gap-3 md:gap-6">
          ...
          <nav className="hidden md:flex items-center gap-6 text-sm font-medium text-slate-600">
            <Link href="/" className={`transition hover:text-brand-600 ${isActive("/") ? "text-brand-700" : ""}`}>Home</Link>

            {!isSeller && user && (
              <Link href="/seller/onboarding" className={`transition hover:text-brand-600 ${isActive("/seller/onboarding") ? "text-brand-700" : ""}`}>
                Become a Seller
              </Link>
            )}
            {isSeller && (
              <Link href="/seller" className={`transition hover:text-brand-600 ${isActive("/seller") ? "text-brand-700" : ""}`}>
                Seller
              </Link>
            )}
            {isAdmin && (
              <Link href="/admin" className={`transition hover:text-brand-600 ${isActive("/admin") ? "text-brand-700" : ""}`}>
                Admin
              </Link>
            )}
            {user && (
              <Link href="/profile" className={`transition hover:text-brand-600 ${isActive("/profile") ? "text-brand-700" : ""}`}>
                Profile
              </Link>
            )}
          </nav>
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          {user ? (
            ...
            <>
              ...
              <Link href="/auth/sign-up" className="hidden lg:block"><Button variant="outline">Sign up</Button></Link>
            </>
          )}
        </div>
      </div>
